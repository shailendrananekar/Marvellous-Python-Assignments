...
def DirectoryRename(DirectoryName, Extension, NewExtension):
    ...
    for FolderName, SubFolderNames, FileNames in os.walk(DirectoryName):
        for fname in FileNames:
            ...
            if fname.endswith(Extension):
                os.rename(os.path.join(FolderName, fname), os.path.join(FolderName, fname.replace(Extension, NewExtension)))
                renamedfile = fname.replace(Extension, NewExtension)
                logs.append("Renamed File is : " + renamedfile)
...
def main():
    ...
    if len(sys.argv) == 2:
        if sys.argv[1] == "--h" or sys.argv[1] == "--H":
            ...
            print("ScriptName.py NameofDirectory fileExtension fileExtension")
